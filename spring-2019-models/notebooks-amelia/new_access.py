...
def get_hh_attributes(household_df): 
    ...
    return pd.Series([income_1st, income_2nd, income_3rd, income_4th, nhwhite_workers, nhblack_workers, nhasian_workers, hispanic_workers, edu_lessthanhs, 
        edu_hs_ged, edu_assoc, edu_bach, edu_grad, renter, owner, census_tract, node_id])
...
def accessibilities_attributes(year, net, dist):
    ...
    'hispanic_workers', 'edu_lessthanhs', 'edu_hs_ged', 'edu_assoc', 'edu_bach', 'edu_grad', 'own', 'rent', 'census_tract', 'node_id']
...
def get_avg_access_geog(geog_df):
    '''get the average access by income quartile by census tract'''
    q1_avg_access = sum((geog_df[geog_df["income_1st"]])["jobs"])/(sum(geog_df["income_1st"])+1e-6)
    q2_avg_access = sum((geog_df[geog_df["income_2nd"]])["jobs"])/(sum(geog_df["income_2nd"])+1e-6)
    q3_avg_access = sum((geog_df[geog_df["income_3rd"]])["jobs"])/(sum(geog_df["income_3rd"])+1e-6)
    q4_avg_access = sum((geog_df[geog_df["income_4th"]])["jobs"])/(sum(geog_df["income_4th"])+1e-6)
    
    rent_avg_access = sum((geog_df[geog_df["own"]])["jobs"])/(sum(geog_df["own"])+1e-6)
    own_avg_access = sum((geog_df[geog_df["rent"]])["jobs"])/(sum(geog_df["rent"])+1e-6)

    white_avg_access = sum((geog_df[geog_df["nhwhite_workers"] > 0])["jobs"])/(sum(geog_df["nhwhite_workers"])+1e-6)
    black_avg_access = sum((geog_df[geog_df["nhblack_workers"] > 0])["jobs"])/(sum(geog_df["nhblack_workers"])+1e-6)
    asian_avg_access = sum((geog_df[geog_df["nhasian_workers"] > 0])["jobs"])/(sum(geog_df["nhasian_workers"])+1e-6)
    hisp_avg_access = sum((geog_df[geog_df["hispanic_workers"] > 0])["jobs"])/(sum(geog_df["hispanic_workers"])+1e-6)

    lessthanhs_avg_access = sum((geog_df[geog_df["edu_lessthanhs"] > 0])["jobs"])/(sum(geog_df["edu_lessthanhs"]))
    hs_ged_avg_access = sum((geog_df[geog_df["edu_hs_ged"] > 0])["jobs"])/(sum(geog_df["edu_hs_ged"]))
    assoc_avg_access = sum((geog_df[geog_df["edu_assoc"] > 0])["jobs"])/(sum(geog_df["edu_assoc"]))
    bach_avg_access = sum((geog_df[geog_df["edu_bach"] > 0])["jobs"])/(sum(geog_df["edu_bach"]))
    grad_avg_access = sum((geog_df[geog_df["edu_grad"] > 0])["jobs"])/(sum(geog_df["edu_grad"]))

    return pd.Series([q1_avg_access, q2_avg_access, q3_avg_access, q4_avg_access, rent_avg_access, own_avg_access, white_avg_access, black_avg_access, 
            asian_avg_access, hisp_avg_access, lessthanhs_avg_access, hs_ged_avg_access, assoc_avg_access, bach_avg_access, grad_avg_access])
 


def accessibilities_by_demog(year, dist, net, geography):
    '''add geography later geography'''
    ...
    #working default is "census_tract for geography"
    ct_access = attr.groupby(geography).apply(get_avg_access_geog)
    ct_access.columns = ['q1_avg_access','q2_avg_access','q3_avg_access','q4_avg_access','rent_avg_access','own_avg_access',
